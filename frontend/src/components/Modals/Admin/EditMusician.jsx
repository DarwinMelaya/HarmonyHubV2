...
const EditMusician = ({ isOpen, onClose, onSuccess, musician }) => {
  ...
  // Update form data when musician prop changes
  useEffect(() => {
    if (musician) {
      setFormData({
        name: musician.name,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/band-artists/${musician._id}`,
        {
          method: "PUT",
          headers: {
      ...
      <div className="bg-gray-800/95 backdrop-blur-md rounded-lg p-6 w-full max-w-md mx-4 border border-gray-700/50 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            ...
            >
              {loading ? "Updating..." : "Update Musician"}
            </button>
            <button
              type="button"
              onClick={handleClose}
